...
func EnqueueMergeTask(uploadVideoUUID string) {
    ...
    default:
        logger.Warn("merge queue full", map[string]interface{}{"upload_video_uuid": uploadVideoUUID})
    }
}
...
func mergeWorker() {
    ...
    for req := range mergeChan {
        ...
        if err := repo.UpdateUploadVideoStatus(ctx, entity.UploadVideoUUID(), vo.UploadVideoStatusSuccess); err != nil {
            logger.Errorf("update status failed", map[string]interface{}{"upload_video_uuid": entity.UploadVideoUUID(), "error": err})
            cancel()
            continue
        }
        EnqueueChunkCleanup(entity.ChunkStoragePath(), int64(entity.TotalChunks()))
        logger.Info("merge success", map[string]interface{}{"upload_video_uuid": entity.UploadVideoUUID()})
        cancel()
    }
}