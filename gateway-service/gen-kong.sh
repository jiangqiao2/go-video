...
if [ "${KONG_ENV}" = "dev" ]; then
  ...
  : "${NOTIFICATION_TARGET:=host.docker.internal:8086}"
  ...
  : "${VIDEO_TARGET:=video-service.go-video.svc:8085}"
   : "${NOTIFICATION_TARGET:=notification-service.go-video.svc:8086}"
  : "${OTEL_EXPORTER_OTLP_ENDPOINT:=http://otel-collector:4318}"
  : "${KONG_TRACE_SAMPLING_RATE:=0.1}"
fi

export UPLOAD_TARGET USER_TARGET RUSTFS_TARGET VIDEO_TARGET NOTIFICATION_TARGET OTEL_EXPORTER_OTLP_ENDPOINT KONG_TRACE_SAMPLING_RATE

envsubst '${UPLOAD_TARGET} ${USER_TARGET} ${RUSTFS_TARGET} ${VIDEO_TARGET} ${NOTIFICATION_TARGET} ${OTEL_EXPORTER_OTLP_ENDPOINT} ${KONG_TRACE_SAMPLING_RATE}' < "$TEMPLATE" > "$OUTPUT"
